...
export function EditTargetClient({ target, habits }: { target: Target; habits: Habit[] }) {
  ...
  const [habitId, setHabitId] = useState(target.habit_id || '');
  const [targetValue, setTargetValue] = useState(String(target.target_value));
  const [frequency, setFrequency] = useState<TargetFrequency>(target.target_frequency);

...
interface HabitDetailClientProps {
  habit: HabitWithFields;
  initialEntry: HabitEntry | null;
  initialHistory: HabitEntry[];
}

export function HabitDetailClient({ habit, initialEntry, initialHistory }: HabitDetailClientProps) {
  const router = useRouter();
  const habitId = habit.id;

  const [selectedDate, setSelectedDate] = useState(today());
  const [values, setValues] = useState<EntryValues>(
    initialEntry ? (initialEntry.values as EntryValues) : {},
  );
  const [history, setHistory] = useState<HabitEntry[]>(initialHistory);
  ...
  // Initialize auto-save with the server-fetched entry
  useEffect(() => {
    const entryValues = initialEntry ? (initialEntry.values as EntryValues) : {};
    initializeSaved(entryValues);
  }, [initialEntry, initializeSaved]);
